...
# script. It uses PROCESS ANCESTRY (not environment variables) to verify
# that publish.py is the caller: it walks the process tree from the hook's
# own PID upward, looking for an ancestor whose command line matches
# `python.* .*scripts/publish.py`. Environment variables are trivially
# spoofable by anyone who can type `FOO=1 git push`; the process tree is
# not. No cheats.
#
# The hook file is BOTH committed to .githooks/pre-push (so every clone has
# a copy) AND rewritten from this template on every publish.py run (so a
# local edit cannot survive the next release). The user still has to run
# `git config core.hooksPath .githooks` once, which publish.py also enforces.

PRE_PUSH_HOOK_TEMPLATE = r"""#!/usr/bin/env bash
...
def ensure_pre_push_hook(git_root: Path) -> None:
    """Install / refresh the pre-push hook and activate core.hooksPath.

    This is called early in the pipeline, BEFORE any validation step. The
    hook verifies its caller via process ancestry — walking up the process
    tree to find a python invocation of scripts/publish.py. During this
    publish.py run the hook will find the ancestor match (this very
    process) and allow the push. Any manual `git push` (no publish.py in
    the ancestry) is refused.
    """
    hooks_dir = git_root / ".githooks"
    hooks_dir.mkdir(exist_ok=True)
    hook_path = hooks_dir / "pre-push"

    # Idempotent: only rewrite if content differs. This keeps the working
    # tree clean on repeat runs while still wiping any local edit that
    # might have added a bypass.
    if not hook_path.exists() or hook_path.read_text(encoding="utf-8") != PRE_PUSH_HOOK_TEMPLATE:
        hook_path.write_text(PRE_PUSH_HOOK_TEMPLATE, encoding="utf-8")
    hook_path.chmod(0o755)

    # Activate .githooks/ as the hook path for this repo. Local config, not
    # committed — publish.py re-asserts it on every run.
    run(
        ["git", "-C", str(git_root), "config", "core.hooksPath", ".githooks"],
        cwd=git_root,
    )
...
def language_bump_version(info: ProjectInfo, new_version: str) -> list[tuple[bool, str]]:
    ...
    if info.has_kind(ProjectKind.PYTHON):
        ...
        results.append(_sync_uv_lock(info.root))
...
def _sync_uv_lock(root: Path) -> tuple[bool, str]:
    """After bumping pyproject.toml, re-run `uv lock` so uv.lock matches the
    new version. Without this, uv.lock drifts one version behind every
    release, leaving a permanent dirty-tree residue."""
    ...
    return True, "uv.lock synced to new version"
...
Examples:
  ...
  %(prog)s --patch --dry-run    # run every validation step fully, then
    ...
    # ── Step 0.5: Install / refresh the pre-push hook ──
    ...
    for name in (
        ...
        "pyproject.toml",
        # uv.lock is regenerated by _sync_uv_lock() during the version bump so
        # it tracks the new version. Without staging it here, the release
        # commit ships a stale lockfile and the next run hits the Step 1
        # dirty-tree branch and cuts an extra `chore: update uv.lock`
        # housekeeping commit (issue #10).
        "uv.lock",
        "package.json",
        "Cargo.toml",
        "CHANGELOG.md",

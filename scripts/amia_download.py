#!/usr/bin/env python3
"""
AMIA Document Download and Storage Manager

Downloads .md files from GitHub issue comments and stores them in the
standardized AMIA folder structure with read-only enforcement.

Usage:
    python amia_download.py download --url URL --task-id TASK_ID --category CATEGORY
    python amia_download.py init --project-root PATH
    python amia_download.py lookup --task-id TASK_ID
    python amia_download.py verify --project-root PATH
"""
...
def init_storage(project_root: Path) -> None:
    """Initialize the AMIA storage directory structure."""
    storage_root = get_storage_root(project_root)

    print(f"Initializing AMIA storage at: {storage_root}")
    ...
    gitkeep.write_text("# AMIA document storage - do not delete this folder\n", encoding="utf-8")
    ...
    print("AMIA storage initialized successfully")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="AMIA Document Download and Storage Manager",
    ...
    if args.command == "init":
        ...
        if args.json:
            ...
            print("\n=== AMIA Storage Verification Report ===\n")

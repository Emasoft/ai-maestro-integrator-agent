...
1. [Common Problems and Solutions](#common-problems-and-solutions)
   - Worktree already exists but directory is empty
   - Port allocation fails with no ports available
   - Registry validation fails with corrupted JSON
   - Health check shows ERROR for all ports
   - Worktree removal says uncommitted changes but there are none
   - Port shows RUNNING but browser says Connection refused
   - Multiple worktrees have same identifier
   - Script fails with Python version too old
   - Git says worktree already exists after script failure
   - Permission denied when running scripts
   - Registry grows too large and slows down scripts
   - Branch doesn't exist and script can't create it
2. [Best Practices Summary](#best-practices-summary)
3. [Quick Reference Command Matrix](#quick-reference-command-matrix)
